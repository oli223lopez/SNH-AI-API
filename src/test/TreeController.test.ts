...
describe('TreeController', () => {
  ...
  describe('createTree', () => {
    it('creates a root node successfully', async () => {
      const result = await treeController.createTree({ label: 'root', parentId: null });
    ...
    it('creates a child node successfully', async () => {
      ...
      const result = await treeController.createTree({ label: 'child', parentId: 1 });
      expect(result).toEqual({
        id: 2,
        label: 'child',
        parentId: 1,
    ...
    it('prevents SQL injection', async () => {
      ...
      const result = await treeController.createTree({ label: maliciousLabel, parentId: null });
